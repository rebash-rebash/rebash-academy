---
title: Projects
description: "Portfolio-ready DevOps projects — mini builds through capstones you can showcase on GitHub."
author: Shaik Basha
category: projects
tags:
  - projects
---

# Projects

Projects turn tutorial skills into portfolio proof. Each one lists prerequisites,
build steps, and what to publish on GitHub.

| Project | Difficulty | Time |
|---------|------------|------|
| [Linux Operations Toolkit](linux-operations-toolkit.md) | advanced | 12–16 hours |
| [Production Linux Operations Platform](linux-production-operations-platform.md) | advanced | 20–30 hours |
| [Linux Server Health Dashboard](linux-server-health-dashboard.md) | intermediate | 8–12 hours |
| [Linux System Information Utility](linux-system-information-utility.md) | beginner | 3–5 hours |
| [Python Cloud Operations Toolkit](python-cloud-operations-toolkit.md) | advanced | 12–18 hours |
| [Production DevOps Automation Platform (Capstone)](python-devops-automation-framework.md) | advanced | 24–40 hours |
| [Python Infrastructure Inventory CLI](python-infra-inventory-cli.md) | intermediate | 8–12 hours |
| [Python Kubernetes Operations Toolkit (moved)](python-k8s-ops-toolkit.md) | advanced | 1 min |
| [Python Log Analysis Tool](python-log-analysis-tool.md) | beginner | 4–6 hours |
| [Python Log Parser (moved)](python-log-parser.md) | beginner | 1 min |
| [Python Platform Engineering Framework](python-platform-engineering-framework.md) | expert | 16–24 hours |
| [Linux Administration Toolkit](shell-linux-administration-toolkit.md) | intermediate | 8–12 hours |
| [Linux Automation Scripts](shell-linux-automation-scripts.md) | beginner | 3–5 hours |
| [Production Shell Automation Framework](shell-production-automation-framework.md) | advanced | 20–30 hours |
| [Production Operations Toolkit](shell-production-operations-toolkit.md) | advanced | 12–16 hours |
| [Status API Portfolio Build](status-api-portfolio.md) | intermediate | 3–4 hours |
