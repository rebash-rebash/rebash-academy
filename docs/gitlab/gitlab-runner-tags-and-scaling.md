---
title: GitLab Runner Tags and Scaling
description: "GitLab Runner Tags and Scaling is essential for engineers who operate GitLab CI in production — not only the team"
...
tags:
  ...
  - scaling
prerequisites:
  - Completed tutorial 5 in this track (or equivalent GitLab CI awareness)
...
GitLab Runner Tags and Scaling is essential for engineers who operate GitLab CI in production — not only the team
that maintains runners. This lesson covers **runner tags, concurrency, autoscaling, and job-to-runner matching** with practical
...
This is **Tutorial 6** in **Module 2: Runners and Configuration** of the REBASH Academy **GitLab CI/CD** track.
...
- [ ] Explain how gitlab runner tags and scaling applies in GitLab CI production pipelines
...
![Architecture diagram for GitLab Runner Tags and Scaling](../assets/images/gitlab-runner-tags-and-scaling.svg)
...
### Production notes for GitLab Runner Tags and Scaling

Teams standardise GitLab CI templates but still integrate with external systems — container
registries, Kubernetes clusters, and cloud OIDC roles. Document **runner tags, concurrency, autoscaling, and job-to-runner matching** in your
...
```bash
mkdir -p ~/rebash-cicd/gitlab-runner-tags-and-scaling && cd ~/rebash-cicd/gitlab-runner-tags-and-scaling
git init -b main
echo "# GitLab Runner Tags and Scaling" > README.md
```
...
## Production Patterns and Deep Dive
        ...
        ### How `GitLab Runner Tags and Scaling` fits in real environments
        ...
        and on-call handovers focused on **runner tags, concurrency, autoscaling, and job-to-runner matching**.
...
```
        ...
        **Scenario:** A teammate merges to `main` and production deploy fails with "permission denied"
        on a step related to **GitLab Runner Tags and Scaling**.
        ...
        Bookmark official GitLab documentation for **GitLab Runner Tags and Scaling**. Note default runner images, quota
...
- GitLab Runner Tags and Scaling is implemented in GitLab CI through `.gitlab-ci.yml`, runners, and merge request workflows
...
1. How does GitLab Runner Tags and Scaling work in GitLab CI?
...
!!! tip "Sample answer — question 1"
    GitLab CI expresses GitLab Runner Tags and Scaling through `.gitlab-ci.yml` jobs, `stages`, and `rules:`. Merge request pipelines use `CI_PIPELINE_SOURCE=merge_request_event`; default branch pipelines use push sources. Map each concept to the predefined variables GitLab injects.
...
- Previous: [GitLab Runners and Executors](gitlab-runners-and-executors.md)
- Next: [Variables, Secrets, and Credentials](variables-secrets-and-credentials.md)
