...
description: "GitLab CI Fundamentals is essential for engineers who operate GitLab CI in production — not only the team"
difficulty: beginner
...
tags:
  ...
  - gitlab-ci
prerequisites:
  - Completed tutorial 2 in this track (or equivalent GitLab CI awareness)
...
GitLab CI Fundamentals is essential for engineers who operate GitLab CI in production — not only the team
that maintains runners. This lesson covers **`.gitlab-ci.yml` stages, rules, includes, and GitLab.com shared runners** with practical
...
This is **Tutorial 3** in **Module 1: Foundations** of the REBASH Academy **GitLab CI/CD** track.
...
- [ ] Explain how gitlab ci fundamentals applies in GitLab CI production pipelines
...
![Architecture diagram for GitLab CI Fundamentals](../assets/images/gitlab-ci-fundamentals.svg)
...
### Production notes for GitLab CI Fundamentals

Teams standardise GitLab CI templates but still integrate with external systems — container
registries, Kubernetes clusters, and cloud OIDC roles. Document **`.gitlab-ci.yml` stages, rules, includes, and GitLab.com shared runners** in your
...
```bash
mkdir -p ~/rebash-cicd/gitlab-ci-fundamentals && cd ~/rebash-cicd/gitlab-ci-fundamentals
git init -b main
echo "# GitLab CI Fundamentals" > README.md
```
...
## Production Patterns and Deep Dive
        ...
        ### How `GitLab CI Fundamentals` fits in real environments
        ...
        and on-call handovers focused on **`.gitlab-ci.yml` stages, rules, includes, and GitLab.com shared runners**.
...
```
        ...
        **Scenario:** A teammate merges to `main` and production deploy fails with "permission denied"
        on a step related to **GitLab CI Fundamentals**.
        ...
        Bookmark official GitLab documentation for **GitLab CI Fundamentals**. Note default runner images, quota
...
- GitLab CI Fundamentals is implemented in GitLab CI through `.gitlab-ci.yml`, runners, and merge request workflows
...
1. How does GitLab CI Fundamentals work in GitLab CI?
...
!!! tip "Sample answer — question 1"
    GitLab CI expresses GitLab CI Fundamentals through `.gitlab-ci.yml` jobs, `stages`, and `rules:`. Merge request pipelines use `CI_PIPELINE_SOURCE=merge_request_event`; default branch pipelines use push sources. Map each concept to the predefined variables GitLab injects.
...
- Previous: [Pipeline Anatomy — Stages, Jobs, and Artifacts](pipeline-anatomy-stages-jobs-and-artifacts.md)
- Next: [GitLab Merge Requests and Pipeline Triggers](gitlab-merge-requests-and-pipeline-triggers.md)
