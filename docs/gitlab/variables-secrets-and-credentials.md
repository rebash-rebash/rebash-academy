---
title: Variables, Secrets, and Credentials
description: "Variables, Secrets, and Credentials is essential for engineers who operate GitLab CI in production — not only the team"
...
tags:
  ...
  - gitlab-ci
prerequisites:
  - Completed tutorial 6 in this track (or equivalent GitLab CI awareness)
...
Variables, Secrets, and Credentials is essential for engineers who operate GitLab CI in production — not only the team
that maintains runners. This lesson covers **CI/CD variables, masked secrets, OIDC, and protected variable scopes** with practical
...
This is **Tutorial 7** in **Module 2: Runners and Configuration** of the REBASH Academy **GitLab CI/CD** track.
...
- [ ] Explain how variables, secrets, and credentials applies in GitLab CI production pipelines
...
![Architecture diagram for Variables, Secrets, and Credentials](../assets/images/variables-secrets-and-credentials.svg)
...
### Core concepts for Variables, Secrets, and Credentials

This tutorial focuses on **CI/CD variables, masked secrets, OIDC, and protected variable scopes** in production GitLab CI pipelines. You will
...
### Production notes for Variables, Secrets, and Credentials

Teams standardise GitLab CI templates but still integrate with external systems — container
registries, Kubernetes clusters, and cloud OIDC roles. Document **CI/CD variables, masked secrets, OIDC, and protected variable scopes** in your
...
```bash
mkdir -p ~/rebash-cicd/variables-secrets-and-credentials && cd ~/rebash-cicd/variables-secrets-and-credentials
git init -b main
echo "# Variables, Secrets, and Credentials" > README.md
```
...
## Production Patterns and Deep Dive
        ...
        ### How `Variables, Secrets, and Credentials` fits in real environments
        ...
        and on-call handovers focused on **CI/CD variables, masked secrets, OIDC, and protected variable scopes**.
...
```
        ...
        **Scenario:** A teammate merges to `main` and production deploy fails with "permission denied"
        on a step related to **Variables, Secrets, and Credentials**.
        ...
        Bookmark official GitLab documentation for **Variables, Secrets, and Credentials**. Note default runner images, quota
...
- Variables, Secrets, and Credentials is implemented in GitLab CI through `.gitlab-ci.yml`, runners, and merge request workflows
...
1. How does Variables, Secrets, and Credentials work in GitLab CI?
...
!!! tip "Sample answer — question 1"
    GitLab CI expresses Variables, Secrets, and Credentials through `.gitlab-ci.yml` jobs, `stages`, and `rules:`. Merge request pipelines use `CI_PIPELINE_SOURCE=merge_request_event`; default branch pipelines use push sources. Map each concept to the predefined variables GitLab injects.
...
- Previous: [GitLab Runner Tags and Scaling](gitlab-runner-tags-and-scaling.md)
- Next: [Triggers, Rules, and Branch Protection](triggers-rules-and-branch-protection.md)
