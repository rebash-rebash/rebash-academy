---
title: Security Scanning in Pipelines
description: "Security Scanning in Pipelines is essential for engineers who operate GitLab CI in production — not only the team"
...
tags:
  ...
  - devsecops
prerequisites:
  - Completed tutorial 13 in this track (or equivalent GitLab CI awareness)
...
Security Scanning in Pipelines is essential for engineers who operate GitLab CI in production — not only the team
that maintains runners. This lesson covers **SAST, container scanning, and policy gates in merge request pipelines** with practical
...
This is **Tutorial 14** in **Module 4: Secure Pipelines** of the REBASH Academy **GitLab CI/CD** track.
...
- [ ] Explain how security scanning in pipelines applies in GitLab CI production pipelines
...
![Architecture diagram for Security Scanning in Pipelines](../assets/images/security-scanning-in-pipelines.svg)
...
### Core concepts for Security Scanning in Pipelines

This tutorial focuses on **SAST, container scanning, and policy gates in merge request pipelines** in production GitLab CI pipelines. You will
...
### Production notes for Security Scanning in Pipelines

Teams standardise GitLab CI templates but still integrate with external systems — container
registries, Kubernetes clusters, and cloud OIDC roles. Document **SAST, container scanning, and policy gates in merge request pipelines** in your
...
```bash
mkdir -p ~/rebash-cicd/security-scanning-in-pipelines && cd ~/rebash-cicd/security-scanning-in-pipelines
git init -b main
echo "# Security Scanning in Pipelines" > README.md
```
...
```bash
trivy fs --severity HIGH,CRITICAL . 2>/dev/null | tail -5 || echo 'Install trivy locally'
```
...
## Production Patterns and Deep Dive
        ...
        ### How `Security Scanning in Pipelines` fits in real environments
        ...
        and on-call handovers focused on **SAST, container scanning, and policy gates in merge request pipelines**.
...
```
        ...
        **Scenario:** A teammate merges to `main` and production deploy fails with "permission denied"
        on a step related to **Security Scanning in Pipelines**.
        ...
        Bookmark official GitLab documentation for **Security Scanning in Pipelines**. Note default runner images, quota
...
- Security Scanning in Pipelines is implemented in GitLab CI through `.gitlab-ci.yml`, runners, and merge request workflows
...
1. How does Security Scanning in Pipelines work in GitLab CI?
...
!!! tip "Sample answer — question 1"
    GitLab CI expresses Security Scanning in Pipelines through `.gitlab-ci.yml` jobs, `stages`, and `rules:`. Merge request pipelines use `CI_PIPELINE_SOURCE=merge_request_event`; default branch pipelines use push sources. Map each concept to the predefined variables GitLab injects.
...
- Previous: [Least-Privilege CI Identities](least-privilege-ci-identities.md)
- Next: [Secret Detection and Supply Chain Basics](secret-detection-and-supply-chain-basics.md)
