---
title: Protected Environments and Approvals
description: "Protected Environments and Approvals is essential for engineers who operate GitLab CI in production — not only the team"
...
tags:
  ...
  - gitlab-ci
prerequisites:
  - Completed tutorial 15 in this track (or equivalent GitLab CI awareness)
...
Protected Environments and Approvals is essential for engineers who operate GitLab CI in production — not only the team
that maintains runners. This lesson covers **protected environments, manual jobs, and deployment windows** with practical
...
This is **Tutorial 16** in **Module 4: Secure Pipelines** of the REBASH Academy **GitLab CI/CD** track.
...
- [ ] Explain how protected environments and approvals applies in GitLab CI production pipelines
...
![Architecture diagram for Protected Environments and Approvals](../assets/images/protected-environments-and-approvals.svg)
...
### Core concepts for Protected Environments and Approvals

This tutorial focuses on **protected environments, manual jobs, and deployment windows** in production GitLab CI pipelines. You will
...
### Production notes for Protected Environments and Approvals

Teams standardise GitLab CI templates but still integrate with external systems — container
registries, Kubernetes clusters, and cloud OIDC roles. Document **protected environments, manual jobs, and deployment windows** in your
...
```bash
mkdir -p ~/rebash-cicd/protected-environments-and-approvals && cd ~/rebash-cicd/protected-environments-and-approvals
git init -b main
echo "# Protected Environments and Approvals" > README.md
```
...
## Production Patterns and Deep Dive
        ...
        ### How `Protected Environments and Approvals` fits in real environments
        ...
        and on-call handovers focused on **protected environments, manual jobs, and deployment windows**.
...
```
        ...
        **Scenario:** A teammate merges to `main` and production deploy fails with "permission denied"
        on a step related to **Protected Environments and Approvals**.
        ...
        Bookmark official GitLab documentation for **Protected Environments and Approvals**. Note default runner images, quota
...
- Protected Environments and Approvals is implemented in GitLab CI through `.gitlab-ci.yml`, runners, and merge request workflows
...
1. How does Protected Environments and Approvals work in GitLab CI?
...
!!! tip "Sample answer — question 1"
    GitLab CI expresses Protected Environments and Approvals through `.gitlab-ci.yml` jobs, `stages`, and `rules:`. Merge request pipelines use `CI_PIPELINE_SOURCE=merge_request_event`; default branch pipelines use push sources. Map each concept to the predefined variables GitLab injects.
...
- Previous: [Secret Detection and Supply Chain Basics](secret-detection-and-supply-chain-basics.md)
- Next: [GitLab Deployment Patterns](gitlab-deployment-patterns.md)
