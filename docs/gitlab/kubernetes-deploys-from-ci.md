---
title: Kubernetes Deploys from CI
description: "Kubernetes Deploys from CI is essential for engineers who operate GitLab CI in production — not only the team"
difficulty: advanced
...
tags:
  ...
  - kubernetes
prerequisites:
  - Completed tutorial 17 in this track (or equivalent GitLab CI awareness)
...
Kubernetes Deploys from CI is essential for engineers who operate GitLab CI in production — not only the team
that maintains runners. This lesson covers **kubeconfig, Helm, namespaces, and progressive delivery hooks** with practical
...
This is **Tutorial 18** in **Module 5: Deploy and Capstone** of the REBASH Academy **GitLab CI/CD** track.
...
- [ ] Explain how kubernetes deploys from ci applies in GitLab CI production pipelines
...
![Architecture diagram for Kubernetes Deploys from CI](../assets/images/kubernetes-deploys-from-ci.svg)
...
### Core concepts for Kubernetes Deploys from CI

This tutorial focuses on **kubeconfig, Helm, namespaces, and progressive delivery hooks** in production GitLab CI pipelines. You will
...
### Production notes for Kubernetes Deploys from CI

Teams standardise GitLab CI templates but still integrate with external systems — container
registries, Kubernetes clusters, and cloud OIDC roles. Document **kubeconfig, Helm, namespaces, and progressive delivery hooks** in your
...
```bash
mkdir -p ~/rebash-cicd/kubernetes-deploys-from-ci && cd ~/rebash-cicd/kubernetes-deploys-from-ci
git init -b main
echo "# Kubernetes Deploys from CI" > README.md
```
...
## Production Patterns and Deep Dive
        ...
        ### How `Kubernetes Deploys from CI` fits in real environments
        ...
        and on-call handovers focused on **kubeconfig, Helm, namespaces, and progressive delivery hooks**.
...
```
        ...
        **Scenario:** A teammate merges to `main` and production deploy fails with "permission denied"
        on a step related to **Kubernetes Deploys from CI**.
        ...
        Bookmark official GitLab documentation for **Kubernetes Deploys from CI**. Note default runner images, quota
...
- Kubernetes Deploys from CI is implemented in GitLab CI through `.gitlab-ci.yml`, runners, and merge request workflows
...
1. How does Kubernetes Deploys from CI work in GitLab CI?
...
!!! tip "Sample answer — question 1"
    GitLab CI expresses Kubernetes Deploys from CI through `.gitlab-ci.yml` jobs, `stages`, and `rules:`. Merge request pipelines use `CI_PIPELINE_SOURCE=merge_request_event`; default branch pipelines use push sources. Map each concept to the predefined variables GitLab injects.
...
- Previous: [GitLab Deployment Patterns](gitlab-deployment-patterns.md)
- Next: [GitLab CI Production Patterns](gitlab-ci-production-patterns.md)
