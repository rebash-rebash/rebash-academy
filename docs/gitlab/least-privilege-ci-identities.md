---
title: Least-Privilege CI Identities
description: "Least-Privilege CI Identities is essential for engineers who operate GitLab CI in production — not only the team"
...
tags:
  ...
  - gitlab-ci
prerequisites:
  - Completed tutorial 12 in this track (or equivalent GitLab CI awareness)
...
Least-Privilege CI Identities is essential for engineers who operate GitLab CI in production — not only the team
that maintains runners. This lesson covers **OIDC to cloud providers, scoped tokens, and job-level permissions** with practical
...
This is **Tutorial 13** in **Module 4: Secure Pipelines** of the REBASH Academy **GitLab CI/CD** track.
...
- [ ] Explain how least-privilege ci identities applies in GitLab CI production pipelines
...
![Architecture diagram for Least-Privilege CI Identities](../assets/images/least-privilege-ci-identities.svg)
...
### Core concepts for Least-Privilege CI Identities

This tutorial focuses on **OIDC to cloud providers, scoped tokens, and job-level permissions** in production GitLab CI pipelines. You will
...
### Production notes for Least-Privilege CI Identities

Teams standardise GitLab CI templates but still integrate with external systems — container
registries, Kubernetes clusters, and cloud OIDC roles. Document **OIDC to cloud providers, scoped tokens, and job-level permissions** in your
...
```bash
mkdir -p ~/rebash-cicd/least-privilege-ci-identities && cd ~/rebash-cicd/least-privilege-ci-identities
git init -b main
echo "# Least-Privilege CI Identities" > README.md
```
...
## Production Patterns and Deep Dive
        ...
        ### How `Least-Privilege CI Identities` fits in real environments
        ...
        and on-call handovers focused on **OIDC to cloud providers, scoped tokens, and job-level permissions**.
...
```
        ...
        **Scenario:** A teammate merges to `main` and production deploy fails with "permission denied"
        on a step related to **Least-Privilege CI Identities**.
        ...
        Bookmark official GitLab documentation for **Least-Privilege CI Identities**. Note default runner images, quota
...
- Least-Privilege CI Identities is implemented in GitLab CI through `.gitlab-ci.yml`, runners, and merge request workflows
...
1. How does Least-Privilege CI Identities work in GitLab CI?
...
!!! tip "Sample answer — question 1"
    GitLab CI expresses Least-Privilege CI Identities through `.gitlab-ci.yml` jobs, `stages`, and `rules:`. Merge request pipelines use `CI_PIPELINE_SOURCE=merge_request_event`; default branch pipelines use push sources. Map each concept to the predefined variables GitLab injects.
...
- Previous: [Parallelism, Matrix, and Pipeline DAGs](parallelism-matrix-and-pipeline-dags.md)
- Next: [Security Scanning in Pipelines](security-scanning-in-pipelines.md)
