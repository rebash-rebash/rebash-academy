---
title: Parallelism, Matrix, and Pipeline DAGs
description: "Parallelism, Matrix, and Pipeline DAGs is essential for engineers who operate GitLab CI in production — not only the team"
...
tags:
  ...
  - gitlab-ci
prerequisites:
  - Completed tutorial 11 in this track (or equivalent GitLab CI awareness)
...
Parallelism, Matrix, and Pipeline DAGs is essential for engineers who operate GitLab CI in production — not only the team
that maintains runners. This lesson covers **parallel jobs, matrix builds, and `needs:` dependency graphs** with practical
...
This is **Tutorial 12** in **Module 3: Build and Quality** of the REBASH Academy **GitLab CI/CD** track.
...
- [ ] Explain how parallelism, matrix, and pipeline dags applies in GitLab CI production pipelines
...
![Architecture diagram for Parallelism, Matrix, and Pipeline DAGs](../assets/images/parallelism-matrix-and-pipeline-dags.svg)
...
### Core concepts for Parallelism, Matrix, and Pipeline DAGs

This tutorial focuses on **parallel jobs, matrix builds, and `needs:` dependency graphs** in production GitLab CI pipelines. You will
...
### Production notes for Parallelism, Matrix, and Pipeline DAGs

Teams standardise GitLab CI templates but still integrate with external systems — container
registries, Kubernetes clusters, and cloud OIDC roles. Document **parallel jobs, matrix builds, and `needs:` dependency graphs** in your
...
```bash
mkdir -p ~/rebash-cicd/parallelism-matrix-and-pipeline-dags && cd ~/rebash-cicd/parallelism-matrix-and-pipeline-dags
git init -b main
echo "# Parallelism, Matrix, and Pipeline DAGs" > README.md
```
...
## Production Patterns and Deep Dive
        ...
        ### How `Parallelism, Matrix, and Pipeline DAGs` fits in real environments
        ...
        and on-call handovers focused on **parallel jobs, matrix builds, and `needs:` dependency graphs**.
...
```
        ...
        **Scenario:** A teammate merges to `main` and production deploy fails with "permission denied"
        on a step related to **Parallelism, Matrix, and Pipeline DAGs**.
        ...
        Bookmark official GitLab documentation for **Parallelism, Matrix, and Pipeline DAGs**. Note default runner images, quota
...
- Parallelism, Matrix, and Pipeline DAGs is implemented in GitLab CI through `.gitlab-ci.yml`, runners, and merge request workflows
...
1. How does Parallelism, Matrix, and Pipeline DAGs work in GitLab CI?
...
!!! tip "Sample answer — question 1"
    GitLab CI expresses Parallelism, Matrix, and Pipeline DAGs through `.gitlab-ci.yml` jobs, `stages`, and `rules:`. Merge request pipelines use `CI_PIPELINE_SOURCE=merge_request_event`; default branch pipelines use push sources. Map each concept to the predefined variables GitLab injects.
...
- Previous: [Artifacts, Caches, and Dependencies](artifacts-caches-and-dependencies.md)
- Next: [Least-Privilege CI Identities](least-privilege-ci-identities.md)
