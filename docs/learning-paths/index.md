---
title: Learning Paths
description: Structured career paths from Linux fundamentals to production-ready cloud engineering.
template: learning-paths.html
difficulty: beginner
estimated_time: "Varies"
author: Shaik Basha
last_updated: "2026-07-28"
category: learning-paths
tags:
  - learning-paths
  - career
comments: false
---

# Learning Paths

Visual learning-path cards are rendered by the site theme.

**Start here in Markdown:** [DevOps Engineer Learning Path](devops-engineer.md) — Linux → Networking → AWS → Git → Docker → Kubernetes → Terraform, plus cheat sheets, labs, and quizzes.
